...
- [ ] **Fix SML --no-default-features build** - Two issues: (1) generated code uses `ooxml_xml::ooxml_bool` but `ooxml-xml` is optional, (2) writer.rs uses feature-gated fields unconditionally. Requires making ooxml-xml required AND feature-gating writer code.
