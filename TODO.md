...
- [ ] **Proper table parsing without re-serialization** - Current implementation serializes `RawXmlElement` back to XML string then re-parses with `FromXml`. Should either: (1) add `FromXml::from_raw_element()` that parses directly from `RawXmlElement`, or (2) extend codegen to recognize `a:graphic/a:graphicData` structure and parse tables inline.
...
- [x] **Add DML extension traits** - Created `ext.rs` with `TextBodyExt`, `TextParagraphExt`, `TextRunExt` traits for generated types.
